...
powershell -NoProfile -Command "$lines=@('**OPTIMIZER LOG**','**User:** %USER%','**IP Address:** %IP%','**Option Used:** %OPT%','**Output:** %MSG%','**Time:** %TIME%','echo ___________________________'); $json=@{content=($lines -join \"`n\")} | ConvertTo-Json; Invoke-RestMethod -Uri '%WEBHOOK%' -Method Post -Body $json -ContentType 'application/json' -ErrorAction SilentlyContinue" 2>nul
